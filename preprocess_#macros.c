// author: jaydattpatel
#include <stdio.h>         // include< .h>  is used to include file from library folder
...
int main( )
{
   ...
   printf("pi=%f\n",pi);
   printf("sum(d1,d2)=%d\t\t",sum(d1,d2));                  printf("sub(d1,d2)=%d\n",sub(d1,d2));
   printf("div(d1,d2)=%d\t\t",div(d1,d2));                  printf("avg(d1,d2)=%d\n",avg(d1,d2));
   printf("square(d1)=%d\t\t",square(d1));                  printf("cube(d1)=%d\n",cube(d1));
   printf("circlearea(f1)=%f\t\t",circlearea(f1));          printf("inc(d1) = %d,inc(d2) = %d\n",inc(d1),inc(d2));
   printf("dec(d1) = %d,dec(d2) = %d\n",inc(d1),dec(d2));
   printf("mer(12,34)=%d\n",mer(12,34));                    printf("sss(Welcome) = ");sss(Welcome);  
   printf("msg = %s\n",c3);                                 msg1(c1,c2,i,d4);  result=1;    
   printf("max(d1,d2)=%d\t\t",max(d1,d2));                  min(d1,d2,result);   printf("min(d1,d2,result)=%d\n",result);
   pwr(d1,d2,i,result); printf("pwr(d1,d2,i,result)=%d\n",result);
   int token34 = 40;
   tokenpaster(34);
